defmodule CatatanBackend.Notes.Crdt.YataTest do
  ...
  describe "initialize/2" do
    test "creates a new Yata instance" do
      yata = Yata.initialize("note-123", "writer-abc")

      assert yata.note_id == "note-123"
      assert yata.writer_id == "writer-abc"
      assert yata.clock == 0
  ...
  describe "increment_clock/1" do
    test "increments the clock by 1" do
      yata = Yata.initialize("note-1", "writer-1")
      assert yata.clock == 0
  ...
  describe "generate_id/1" do
    test "returns tuple of writer_id and clock" do
      yata = Yata.initialize("note-1", "my-writer")
      assert Yata.generate_id(yata) == {"my-writer", 0}

      yata = Yata.increment_clock(yata)
      assert Yata.generate_id(yata) == {"my-writer", 1}
    end
  end
  ...
  describe "insert/4" do
    test "inserts element at the beginning (nil origin)" do
      ...
      assert yata.clock == 1
      assert Map.has_key?(yata.elements, "writer-1:1")
      assert yata.state_vector == %{"writer-1" => 1}
    end
    ...
    test "inserts multiple elements sequentially" do
      ...
      assert el_a.id == {"writer-1", 1}
      assert el_b.id == {"writer-1", 2}
      assert el_c.id == {"writer-1", 3}

      assert el_b.origin == {"writer-1", 1}
      assert el_c.origin == {"writer-1", 2}
  ...
  describe "delete/2" do
    test "marks an element as deleted" do
      ...
      {:ok, yata, deleted_element} = Yata.delete(yata, element_key)
    ...
    test "returns error when element not found" do
      ...
      assert {:error, :not_found} = Yata.delete(yata, "nonexistent:1")
    end
  end
  ...
  describe "integrate/2" do
    test "integrates a new remote element" do
      ...
      assert Map.has_key?(yata.elements, "writer-2:1")
      assert yata.state_vector["writer-2"] == 1
    end
  ...
  describe "to_list/1" do
    ...
    test "excludes deleted elements" do
      ...
      # Delete "b"
      {:ok, yata, _} = Yata.delete(yata, Element.encode_id(el_b.id))
  ...
  describe "concurrent insertions" do
    test "two users insert at same position - converges regardless of order" do
      ...
      yata_a = Yata.initialize("note-1", "writer-a")
      yata_b = Yata.initialize("note-1", "writer-b")
    ...
    test "concurrent insertions after same origin - converges" do
      ...
      # User B also inserts after X (different writer)
      yata_b = %{yata | writer_id: "writer-b"}
    ...
    test "interleaved typing from two users converges" do
      ...
      base = Yata.initialize("note-1", "writer-base")

      # Writer A types "A" then "B"
      writer_a = %{base | writer_id: "writer-a"}
      {writer_a, el_a1} = Yata.insert(writer_a, nil, nil, "A")
      {_writer_a, el_a2} = Yata.insert(writer_a, el_a1.id, nil, "B")

      # Writer B types "1" then "2" (also at beginning, concurrently)
      writer_b = %{base | writer_id: "writer-b"}
      {writer_b, el_b1} = Yata.insert(writer_b, nil, nil, "1")
      {_writer_b, el_b2} = Yata.insert(writer_b, el_b1.id, nil, "2")
    ...
    test "multiple insertions at same position with right_origin" do
      ...
      # Two different writers insert between "a" and "c"
      el_x = %Element{
        id: {"writer-x", 1},
        origin: el_a.id,
  ...
  describe "sort_conflicting/2" do
    test "sorts elements with same origin by right_origin and id" do
      ...
      el_a = %Element{id: {"writer-a", 1}, origin: nil, right_origin: nil, content: "A"}
      el_b = %Element{id: {"writer-b", 1}, origin: nil, right_origin: nil, content: "B"}
      ...
      assert ids == [{"writer-a", 1}, {"writer-b", 1}]
    end
  end
end
